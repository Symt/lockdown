...
function tableGen(json) {
    ...
    for (var i = 0; i < json.available_downloads.length; i++) {
      rows.push(
        <tr key={i}><td>{json.available_downloads[i]["title"]}</td><td><Event eventAction={"{download_file|/tmp/|"+json.available_downloads[i]['link']+"|"+json.available_downloads[i]['application_name'] + "}"} eventTitle={json.available_downloads[i]['application_name']}/></td></tr>
      )
    }
    return rows;
}
